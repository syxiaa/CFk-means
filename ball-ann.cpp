...
void run(MatrixOur &dataset, MatrixOur &centroids) {
    ...
    VectorOur delta(k);
    VectorOur x_points(dataset_rows);
    VectorOur x_centroids(k);

    vector<unsigned int> now_data_index;
    vector<unsigned int> old_now_index;
    vector<unsigned int> rela_data_index;
        ...
        if (new_centroids != old_centroids) {
            ...
            for (unsigned int now_ball = 0; now_ball < k; now_ball++) {
                ...
                for (unsigned int i = 0; i < data_num; i++) {
                    ...
                    secVal = -1;
                    firIndex = 0;
                    for (int j = 0; j < now_centers_rows; j++) {

                        if (x_centroids(clusters_neighbors_index[now_ball][j]) >
                            point_center_dist[now_ball][rela_data_index[i]] + x_points(now_data_index[i]) ||
                            x_centroids(clusters_neighbors_index[now_ball][j]) <
                            point_center_dist[now_ball][rela_data_index[i]] - x_points(now_data_index[i])) {
                            continue;
                        }

                        temp = (dataset.row(now_data_index[i]) - now_centers.row(j)).squaredNorm();
                        cal_dist_num += 1;

                        if (firVal > temp || firVal == -1) {
                            secVal = firVal;
                            firVal = temp;
                            firIndex = j;
                        }
                        else if (secVal > temp || secVal == -1) {
                            secVal = temp;
                        }
                    }
...
inline MatrixOur update_centroids(MatrixOur &dataset, ClusterIndexVector &clusters_point_index, unsigned int k,
                                  ...
                                  MatrixOur &old_centroids, VectorOur &x_centroids) {
    /*
    ...
    */
    ...
    for (unsigned int i = 0; i < k; i++) {
        ...
        if (flag(i) != 0 || iteration_counter == 1) {
            ...
            x_centroids(i) = sqrt(new_c.row(i).squaredNorm());
...
void initialize(MatrixOur &dataset, MatrixOur &centroids, VectorOur &labels, ClusterIndexVector &clusters_point_index,
                ...
                VectorOur &x_points, VectorOur &x_centroids) {
    /*
    ...
    */
    x_points = (dataset.rowwise().squaredNorm()).array().sqrt();
    x_centroids = (centroids.rowwise().squaredNorm()).array().sqrt();
